...
void main()
{
	...
	// Rashly assume that all 4 normals are meant to be 
	// within 90degrees of each other:
	
	if( dot( N042, N021 ) < 0.0 )
		N021 = -N021;
		
	if( dot( N042, N243 ) < 0.0 )
		N243 = -N243;
		
	if( dot( N042, N405 ) < 0.0 )
		N405 = -N405;
		
		
	if( N042.z * N021.z < 0.0 )
	{
		gl_Position = u_projectionMatrix * vec4( V0, 1. );
		EmitVertex();
		gl_Position = u_projectionMatrix * vec4( V2, 1. );
		EmitVertex();
		EndPrimitive();
	}	
	...
	if( N042.z * N243.z < 0.0 )
	{
		gl_Position = u_projectionMatrix * vec4( V2, 1. );
		EmitVertex();
		gl_Position = u_projectionMatrix * vec4( V4, 1. );
		EmitVertex();
		EndPrimitive();
	}
